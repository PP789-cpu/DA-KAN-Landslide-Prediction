...
CONFIG = {
    ...
    'PRETRAINED_MODEL_PATH': 'tcn_kan_zd1_trend_model.h5',
    'RESULT_XLS': 'ZG111_Trend_Transfer_Final.xlsx'
}
...
def train_single_transfer_model(X_train, Y_train, X_test, Y_test, seed, run_id):
    ...
    temp_path = f"temp_trend_{run_id}.weights.h5"
    ...
    model.fit(X_train, Y_train, epochs=300, batch_size=CONFIG['BATCH_SIZE_TGT'],
...
def run_transfer_target():
    try:
        ...
        feature_data = df[CONFIG['FEAT_COL']].values
        ...
        with pd.ExcelWriter(CONFIG['RESULT_XLS'], engine='openpyxl') as writer:
            for r in ratios:
                ...
                for i in range(n_ensemble):
                    p = train_single_transfer_model(X_train, Y_train, X_test, Y_test,
                                                    seed=100 + i, run_id=f"Trend_R{r}_{i}")
                    preds.append(p)
            ...
            plt.plot(Y_test_real.flatten(), label='Actual Trend', color='blue', linestyle='-')
            plt.plot(Y_pred_real.flatten(), label='Predicted Trend', color='red', linestyle='--')
            plt.title(f'Target Domain Trend Prediction (R2: {r2:.4f})')
